...
const SIGNATURES: DaxFunctionSignature[] = [
  ...
  fn('SUM', 'aggregation', [column('column')], 'scalar', 'Adds up every value in a column.'),
  fn('AVERAGE', 'aggregation', [column('column')], 'scalar', 'The mean of a column, ignoring blanks.'),
  fn('MIN', 'aggregation', [column('column')], 'scalar', 'The smallest value in a column.'),
  fn('MAX', 'aggregation', [column('column')], 'scalar', 'The largest value in a column.'),
  fn('COUNT', 'aggregation', [column('column')], 'scalar', 'Counts the non-blank values in a column.'),
  fn('COUNTA', 'aggregation', [column('column')], 'scalar', 'Counts values in a column, including text.'),
  fn('COUNTBLANK', 'aggregation', [column('column')], 'scalar', 'Counts the blanks in a column.'),
  fn('DISTINCTCOUNT', 'aggregation', [column('column')], 'scalar', 'Counts how many different values a column holds.'),
  fn('COUNTROWS', 'aggregation', [table('table', true)], 'scalar', 'Counts the rows in a table, after any filters.'),

  // Iterators, which evaluate an expression row by row.
  fn('SUMX', 'iterator', [table('table'), scalar('expression')], 'scalar', 'Evaluates an expression for each row, then adds the results.'),
  fn('AVERAGEX', 'iterator', [table('table'), scalar('expression')], 'scalar', 'Evaluates an expression for each row, then averages the results.'),
  fn('MINX', 'iterator', [table('table'), scalar('expression')], 'scalar', 'The smallest result of an expression across rows.'),
  fn('MAXX', 'iterator', [table('table'), scalar('expression')], 'scalar', 'The largest result of an expression across rows.'),
  fn('COUNTX', 'iterator', [table('table'), scalar('expression')], 'scalar', 'Counts non-blank results of an expression across rows.'),
  ...
  // Filter context.
  fn('CALCULATE', 'filter', [scalar('expression'), any('filter', true)], 'scalar', 'Evaluates an expression with the filter context modified.', { variadic: true }),
  ...
  fn('FILTER', 'filter', [table('table'), scalar('condition')], 'table', 'Keeps only the rows of a table where a condition holds.'),
  fn('ALL', 'filter', [any('tableOrColumn', true)], 'table', 'Removes filters from a table or columns.', { variadic: true }),
  fn('ALLEXCEPT', 'filter', [table('table'), column('column')], 'table', 'Removes every filter from a table except those on the named columns.', { variadic: true }),
  ...
  fn('VALUES', 'filter', [any('tableOrColumn')], 'table', 'The distinct values visible in the current filter context.'),
  fn('DISTINCT', 'filter', [any('tableOrColumn')], 'table', 'The distinct values of a column or rows of a table.'),
  fn('RELATED', 'filter', [column('column')], 'scalar', 'Fetches a value from the one side of a relationship.'),
  ...
  // Logic.
  fn('IF', 'logical', [scalar('condition'), any('then'), any('else', true)], 'any', 'Chooses between two results.'),
  fn('SWITCH', 'logical', [any('expression'), any('value'), any('result')], 'any', 'Matches an expression against a list of values.', { variadic: true }),
  fn('AND', 'logical', [scalar('a'), scalar('b')], 'scalar', 'True when both arguments are true.'),
  fn('OR', 'logical', [scalar('a'), scalar('b')], 'scalar', 'True when either argument is true.'),
  fn('NOT', 'logical', [scalar('value')], 'scalar', 'Reverses a true/false value.'),
  fn('IFERROR', 'logical', [any('value'), any('alternate')], 'any', 'Returns an alternative when the first argument errors.'),
  fn('ISBLANK', 'info', [any('value')], 'scalar', 'True when a value is blank.'),
  fn('ISERROR', 'info', [any('value')], 'scalar', 'True when an expression errors.'),
  fn('COALESCE', 'logical', [any('value')], 'any', 'The first argument that is not blank.', { variadic: true }),
  fn('BLANK', 'info', [], 'scalar', 'The blank value.'),

  // Arithmetic.
  fn('DIVIDE', 'math', [scalar('numerator'), scalar('denominator'), scalar('alternateResult', true)], 'scalar', 'Divides, returning an alternative instead of an error when dividing by zero.'),
  fn('ROUND', 'math', [scalar('number'), scalar('digits')], 'scalar', 'Rounds to a number of decimal places.'),
  fn('ROUNDUP', 'math', [scalar('number'), scalar('digits')], 'scalar', 'Rounds away from zero.'),
  fn('ROUNDDOWN', 'math', [scalar('number'), scalar('digits')], 'scalar', 'Rounds towards zero.'),
  fn('ABS', 'math', [scalar('number')], 'scalar', 'The magnitude of a number, without its sign.'),
  fn('CEILING', 'math', [scalar('number'), scalar('significance')], 'scalar', 'Rounds up to a multiple.'),
  fn('FLOOR', 'math', [scalar('number'), scalar('significance')], 'scalar', 'Rounds down to a multiple.'),
  fn('INT', 'math', [scalar('number')], 'scalar', 'Truncates to a whole number.'),
  fn('POWER', 'math', [scalar('number'), scalar('power')], 'scalar', 'Raises a number to a power.'),
  fn('SQRT', 'math', [scalar('number')], 'scalar', 'The square root of a number.'),
  fn('MOD', 'math', [scalar('number'), scalar('divisor')], 'scalar', 'The remainder after division.'),
  ...
  // Text.
  fn('CONCATENATE', 'text', [scalar('a'), scalar('b')], 'scalar', 'Joins two strings.'),
  fn('LEFT', 'text', [scalar('text'), scalar('count', true)], 'scalar', 'The first characters of a string.'),
  fn('RIGHT', 'text', [scalar('text'), scalar('count', true)], 'scalar', 'The last characters of a string.'),
  fn('MID', 'text', [scalar('text'), scalar('start'), scalar('count')], 'scalar', 'Characters from the middle of a string.'),
  fn('LEN', 'text', [scalar('text')], 'scalar', 'How many characters a string has.'),
  fn('UPPER', 'text', [scalar('text')], 'scalar', 'Converts text to upper case.'),
  fn('LOWER', 'text', [scalar('text')], 'scalar', 'Converts text to lower case.'),
  fn('TRIM', 'text', [scalar('text')], 'scalar', 'Removes leading and trailing spaces.'),
  fn('SUBSTITUTE', 'text', [scalar('text'), scalar('old'), scalar('new')], 'scalar', 'Replaces one piece of text with another.'),
  fn('FORMAT', 'text', [scalar('value'), scalar('format')], 'scalar', 'Formats a value as text.'),

  // Dates.
  fn('YEAR', 'date', [scalar('date')], 'scalar', 'The year of a date.'),
  fn('MONTH', 'date', [scalar('date')], 'scalar', 'The month of a date, 1 to 12.'),
  fn('DAY', 'date', [scalar('date')], 'scalar', 'The day of the month.'),
  fn('QUARTER', 'date', [scalar('date')], 'scalar', 'The quarter of a date, 1 to 4.'),
  fn('WEEKNUM', 'date', [scalar('date'), scalar('returnType', true)], 'scalar', 'The week number of a date.'),
  fn('TODAY', 'date', [], 'scalar', "Today's date."),
  fn('NOW', 'date', [], 'scalar', 'The current date and time.'),
  fn('DATE', 'date', [scalar('year'), scalar('month'), scalar('day')], 'scalar', 'Builds a date from its parts.'),
